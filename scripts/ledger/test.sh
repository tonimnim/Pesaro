...
for ledger_file in ca.crt client.root.crt client.root.key client.ledger_runtime.crt client.ledger_runtime.key client.reconciliation_runtime.crt client.reconciliation_runtime.key; do
...
export PESAR_RECONCILIATION_TEST_ADMIN_URL="postgresql://root@localhost:26277/pesaro_reconciliation?sslmode=verify-full&sslrootcert=$ledger_certs/ca.crt&sslcert=$ledger_certs/client.root.crt&sslkey=$ledger_certs/client.root.key"
export PESAR_RECONCILIATION_TEST_RUNTIME_URL="postgresql://reconciliation_runtime@localhost:26277/pesaro_reconciliation?sslmode=verify-full&sslrootcert=$ledger_certs/ca.crt&sslcert=$ledger_certs/client.reconciliation_runtime.crt&sslkey=$ledger_certs/client.reconciliation_runtime.key"
...
PESAR_RECONCILIATION_ADMIN_URL="$PESAR_RECONCILIATION_TEST_ADMIN_URL" go run ./services/reconciliation/cmd/reconciliation-admin -synthetic | tee "$PESAR_LEDGER_EVIDENCE_DIR/inbox-migrate.log"
go test -count=1 -v "${ledger_race[@]}" ./services/ledger/... ./services/reconciliation/... ./tests/eventdelivery | tee "$PESAR_LEDGER_EVIDENCE_DIR/tests.log"
if "$ledger_restart"; then
  ledger_artifact="$PESAR_LEDGER_EVIDENCE_DIR/restart-instruction.json"
  PESAR_LEDGER_RESTART_PREPARE="$ledger_artifact" go test -count=1 -run '^Test(Database|Inbox)RestartPrepare$' -v ./services/ledger/internal/app ./services/reconciliation/internal/inbox | tee "$PESAR_LEDGER_EVIDENCE_DIR/restart-prepare.log"
  bash scripts/ledger/local-db.sh crash
  bash scripts/ledger/local-db.sh start
  PESAR_LEDGER_RESTART_RECOVER="$ledger_artifact" go test -count=1 -run '^Test(Database|Inbox)RestartRecover$' -v ./services/ledger/internal/app ./services/reconciliation/internal/inbox | tee "$PESAR_LEDGER_EVIDENCE_DIR/restart-recover.log"
fi
{
  go version
  git rev-parse HEAD
