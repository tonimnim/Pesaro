...
try {
    ...
    foreach ($file in @('ca.crt', 'client.root.crt', 'client.root.key', 'client.ledger_runtime.crt', 'client.ledger_runtime.key')) {
    ...
    if ($RestartDatabase) {
        ...
        try {
            & go test -count=1 -run '^TestDatabaseRestartPrepare$' -v ./services/ledger/internal/app | Tee-Object -FilePath (Join-Path $env:PESAR_LEDGER_EVIDENCE_DIR 'restart-prepare.log')
        ...
        try {
            & go test -count=1 -run '^TestDatabaseRestartRecover$' -v ./services/ledger/internal/app | Tee-Object -FilePath (Join-Path $env:PESAR_LEDGER_EVIDENCE_DIR 'restart-recover.log')
